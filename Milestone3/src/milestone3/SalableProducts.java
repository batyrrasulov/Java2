...
import milestone3.SalableProducts;

public class SalableProducts {
	//initialized the varialbles
	static String name;
	...
	//menu to display product categories
		...
		static void weapons() {
			...
			if (wpn == 1) {
				...
				if (ansSD == 'y' || ansSD == 'Y' ) {
					...
					InventoryManager.removeSteelDagger(steelDaggerPurch); //removes from inventory with an integration with Inventory Manager
					StoreFront.menu();
				...
				if(ansTS == 'y' || ansTS == 'Y') {
					...
					InventoryManager.removeExecutionerAxe(executionerAxePurch); //removes from inventory with an intergration with Inventory Manager
					StoreFront.menu();
				} else //if decides not to buy brings back to product menu
					products();
			}
			else if (wpn == 3) {
				InventoryManager.returnSteelDagger(steelDaggerPurch);
				InventoryManager.returnExecutionerAxe(executionerAxePurch);
		...
		static void armor() {
			...
			if (armr == 1) {
				...
				if (ansTL == 'y' || ansTL == 'Y') {
					...
					InventoryManager.removeTannedLeather(tannedLeatherPurch); //removes from inventory with an integration with Inventory Manager
					StoreFront.menu();
				} else //if decides not to buy brings back to product menu
					products();
			}
				...
				if (ansS == 'y' || ansS == 'Y') {
					...
					InventoryManager.removeSteeler(steelerPurch); //removes from inventory with an integration with Inventory Manager
					StoreFront.menu();
				} else //if decides not to buy brings back to product menu
					products();
			}
			else if (armr == 3) {
				InventoryManager.returnTannedLeather(tannedLeatherPurch);
				InventoryManager.returnSteeler(steelerPurch);
				System.out.println("Unfortunately no purchase has been made today, please come again!");
				System.exit(0);
			}
			else {
		...
		static void health() {
			...
			if (hlth == 1) {
				HealthProducts.add100(); //calls add100 method from HealthProducts class
				...
				if(ansS == 'y' || ansS == 'Y') {
					...
					InventoryManager.removeAdd100(add100Purch); //removes from inventory with an integration with Inventory Manager
					StoreFront.menu();
				} else //if decides not to buy brings back to product menu
					products();
			}
			else if (hlth == 2) {
				HealthProducts.add250(); //calls add250 method from HealthProducts class
				...
				if(ansTS == 'y' || ansTS == 'Y') {
					...
					InventoryManager.removeAdd250(add250Purch); //removes from inventory with and integration with Inventory Manager
					StoreFront.menu();
				} else //if decides not to buy brings back to product menu
					products();
			}
			else if (hlth == 3) {
				InventoryManager.returnAdd100(add100Purch);
				InventoryManager.returnAdd250(add250Purch);
				System.out.println("Unfortunately no purchase has been made today, please come again!");
				System.exit(0);
			}
			else {
